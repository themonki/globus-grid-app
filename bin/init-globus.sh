...
if [ "$SHARE_FOLDER_ACTIVE" != "true" ]; then
	...
	if [ -e "$FILE" ]; then
		...
		result=$(grep "user_maquina_local" < "$FILE")
		var=${result#*=}
		usr_file=${var//"\""/""}
		result=$(grep "pass_maquina_local" < "$FILE")
		var=${result#*=}
		pass_file=${var//"\""/""}
		result=$(grep "ip_maquina_local" < "$FILE")
		var=${result#*=}
		ip_file=${var//"\""/""}
		result=$(grep "path_project_vagrant" < "$FILE")
		var=${result#*=}
		dir_file=${var//"\""/""}
...
do
	...
	vagrant up "$node_name" --color >> "$LOG_FILE"
done
...
{
	"$DIR_BIN"/getCredencial.sh;

	"$DIR_BIN"/restart-globus.sh;
